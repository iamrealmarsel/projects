...
$('.header__nav .fa-close, .ovl, .ovl_menu').click(function() {
  close();
});
...
$('.faq__subtitle').on('click', function() {
  ...
  $(this).siblings('.faq__cnt').slideToggle();
});


// табы
$('.tariffs__main').nextAll().hide();
$('.tariffs__tabs li').on('click', function(e) {
  e.preventDefault();
  var i = $(this).index();
  $(this).siblings().removeClass('active')
  $(this).addClass('active');
  $('.tariffs__main').hide().eq(i).show();
});
...
if ($('.window').length) {
  ...
  $('.window__slider').on('afterChange', function(event, slick, currentSlide){
    var num = currentSlide + 1;
    var res = "0" + num;
    $('.window__num').html(res);
  });
...
if ($(window).width() >= 992) {
  ...
  var myMap = new ymaps.Map("map", {
    center: [43.565572, 39.747747], 
    zoom: 16
  });

}

  var myPlacemark = new ymaps.Placemark([43.565572, 39.747747], { 
    // balloonContent: '',
    iconCaption: 'Курортный проспект, 84/2'
  });
